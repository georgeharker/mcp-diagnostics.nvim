...
function M.setup(user_config)
  ...
  if not has_mcphub then
    local msg = "[MCP Diagnostics] mcphub.nvim is required but not found. Please install it first."
    vim.notify(msg, vim.log.levels.ERROR)
    return false
  end

  if config.debug then
    vim.notify("[MCP Diagnostics] Starting mcphub setup with config: " .. vim.inspect(config), vim.log.levels.DEBUG)
  end
  ...
  if config.auto_register then
    M.register_with_mcphub(mcphub, config, core)
  end
...
function M.check_status()
  vim.notify("Use :checkhealth mcp-diagnostics for comprehensive status information", vim.log.levels.INFO)
  vim.cmd("checkhealth mcp-diagnostics")
end

-- Register with mcphub.nvim using incremental approach
function M.register_with_mcphub(mcphub, config, core)
  ...
  local success, result = pcall(function()
    ...
    if config.enable_diagnostics or config.enable_lsp then
      tools.register_all(mcphub, config.server_name, core, config)
    end

    -- Register resources
    if config.enable_diagnostics then
      resources.register_all(mcphub, config.server_name, core, config)
    end

    -- Register prompts
    if config.enable_prompts then
      prompts.register_all(mcphub, config.server_name, core, config)
    end
  ...
  vim.notify(
    string.format("[MCP Diagnostics] Registered '%s' with mcphub.nvim%s%s", config.displayName, approve_msg, reload_msg),
    vim.log.levels.INFO
  )
